...
- **Span:** 60 consecutive "salary months", each anchored on the user's salary day (day of month they get paid, default 25th, configurable during Setup). The first month is anchored to the user's currently-active salary cycle at registration time. Persisted per-user as `startDate` + `salaryDay` on the financial document; legacy accounts created before these fields existed fall back to the 25th of December 2025 so their existing month-indexed data stays aligned.
- **Month naming:** Each salary cycle (payday → day before next payday) is labeled after the calendar month containing the TRUE majority of its days by actual day-count, not the payday's own month and not just "whichever month the cycle's last day falls in". E.g. paid on the 25th: the cycle June 25 – July 24 is labeled "Jul 2026" (24 July days vs 6 June days). Paid on the 5th: the cycle July 5 – Aug 4 is also labeled "Jul 2026" (27 July days vs only 4 August days) - not "Aug 2026". A payday on the 1st keeps the same-month label since the whole cycle falls in one calendar month. The month-select dropdown additionally shows the payday date and an arrears "for" qualifier in parentheses - the calendar month containing the majority of days in the *preceding* pay period, by the same day-count approach (not a flat "payday month minus one"), e.g. both a payday on the 25th and one on the 5th are correctly labeled "for Jun" when the majority of their respective pay periods falls in June: "Jul 2026 (for Jun, paid 25 Jun)" and "Jul 2026 (for Jun, paid 5 Jul)".
- **Default selected month:** On load, the planner selects whichever salary month is currently active - i.e. the most recent month whose salary day has already occurred - not always the first month. Example: if paid on the 25th and today is July 10, the active month is still June (July's salary hasn't arrived yet).
- **Month status indicator:** The dropdown marks each month as ✅ past (cycle fully elapsed), 🟢 current (today falls within this cycle), or 🔴 not yet (payday hasn't happened). This is independent of which month the user has manually navigated to.
...
- **Modal shows:**
  ...
  - A live "Entered X / Y available" info line. Any difference between the entered amounts and a month's real available balance (including the selected month) is automatically absorbed by the percentage split below — entering unbalanced numbers does **not** block Apply, since the split (default 100% Savings) exists precisely to take up that slack. Each bucket also offers a one-click "put the remaining ±N here" shortcut for users who prefer to balance by hand.
...
- **Follow-up bug (fixed same day):** the modal's Apply validation additionally required the raw entered amounts to already sum to the selected month's available balance, ignoring the fact that the percentage split absorbs the difference for every month **including the selected one**. This blocked exactly the intended workflow (e.g. adjust Groceries/Entertainment, leave the default 100% Savings split, apply to all upcoming months) with a false "3 budgets don't add up" error. Fixed by removing that hard requirement — Apply now only requires non-negative targets, a distribution that totals 100%, and at least one editable month in range; the balance info line is now purely informational.
