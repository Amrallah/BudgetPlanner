    ...
    <div className="bg-card rounded-2xl border border-border shadow-sm p-4 sm:p-5 mb-4">
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm sm:text-base font-semibold tracking-tight text-foreground">Variable Expenses</h3>
        </div>
      </div>
      <div className="space-y-2.5 sm:space-y-3">
        {fields.map(field => {
          ...
          return (
          <div key={field.type} className={`p-3 sm:p-4 bg-muted/50 rounded-xl border border-border ${isLocked ? 'opacity-70' : ''}`} aria-disabled={isLocked}>
            ...
            <div className="text-xs text-muted-foreground mt-2 flex items-center gap-3">
              <div>
                ...
                {field.recentTransactions && field.recentTransactions.length > 0 ? (
                  ...
                  <span className="text-muted-foreground ml-2">No recent transactions</span>
                )}
              </div>
              <button
                onClick={() => !isLocked && onOpenHistory(field.type)}
                disabled={isLocked}
                className="ml-auto bg-card border border-border text-foreground/90 px-2.5 py-1 rounded-md hover:bg-muted/50 text-xs disabled:opacity-60 disabled:cursor-not-allowed"
              >
